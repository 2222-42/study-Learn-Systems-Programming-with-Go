...
データの入出力や加工を行う関数を書く場合、選択肢は3つあります。
- ファイル名を受け取る
- io.Writerやio.Readerを受け取る <- これが望ましい
- バイト列を受け取る

なるべくio.Writer やio.Reader を関数の引数として扱う。
- ファイル名を受け取って処理したい場合も、ヘルパー関数を1つ作ってラッパーを用意する
  - ファイル名を受け取ると、ネットワーク経由やオンメモリで作った内容（テストコード内のデータも含む）を直接読み書きできないので、
  一度ファイルに書き出すといった余計なコードが必要になってしまい
- バイト列を受け取りたい場合も、bytes.Bufferで同様に扱えます
  - 柔軟性という点ではioutilの各種関数を1 つ使うことでラッパーライブラリを簡単に作れるが、書き込む量と同じメモリ量が必要になるデメリットがある
    - ioutil.ReadFile() などは、一括でファイルを読み込んでバイト配列にする
    - ioutil.WriteFile() も、書き込む内容をバイト配列で用意する
      - これらの重い処理をOS側が処理することになる。
  ― io.Writerやio.Readerの方が小さいサイズのデータ単位で扱うことができるからそちらのほうがいい
