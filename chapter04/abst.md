...
複数のチャンネルを1つの関数であつかうと、ブロックされ続けたり、並列処理の管理が大変になる。

― ブロックする複数のチャネルを同時に並列で待ち受け、データが到着したチャネルから順に取り出して処理する
― ブロックする複数のチャネルの書き込みが完了するのを並列で待ち受け、データが先に送信できたチャネルにのみデータを投入する

こういったことをやりたいなら、
-> `select`文を使う。

`select` は一度トリガーすると終わるから、`for`ループの中に入れる。
...
case 文には、必要な数だけチャネルの読み込みコードを列挙しとけば、複数のチャネルを1つの関数で扱うさいに実現したいことができる

default使えば、いずれに該当しない場合に実施することも書けるよ

### 4.2.5 コンテキスト

コンテキストは、
深いネストの中、あるいは派生ジョブなどがあって複雑なロジックの中でも、
正しく終了やキャンセル、タイムアウトが実装できるようにする仕組み

(すごいざっくりした説明だけど、確かにそれはそう。

並列処理については "currency in go" がよいか。
しかし、当該の本でもそれほど詳しく書いていたわけではないので、別途別の書籍や、公式ドキュメントを閲覧する必要があるだろう。
)
