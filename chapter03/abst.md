...
バイナリ解析の場合は、
- 読み込むバイト数が固定であったり、
- 可変長データの場合も読み込むバイト数や個数などが事前に明示されていることがほとんど
...
- 次にチャンク名の書き込み、
- 本体の書き込み、
- 最後にCRCの計算と、binary.Write() による書き込み
  - 巡回冗長検査（じゅんかいじょうちょうけんさ、英: Cyclic Redundancy Check, CRC）は、誤り検出符号の一種で、主にデータ転送などに伴う偶発的な誤りの検出によく使われている
...
バイナリ解析とは異なり、テキスト解析ではデータ長が決まらず、スキャンしながら区切りを探すしかなく、探索しながら読み込んでいく必要がある

### 3.6.1 改行／単語で区切る

テキスト解析の基本は改行区切り

io.Reader による入力ではbufio.Readerを使うという手があり、そちらのほうが全体を読み込んで開業を探すより、比較的シンプル

(bufio.Readerは読み込んだ文字を戻すこともできるため、テキストの構文解析器を自前で作る際のベースにできます。)

bufio.Readerを使ったケースの結果は以下の通りであり、改行記号が残っている：
...
bufio.Scannerは終端を気にせず、err handlingが不要になるが、分割文字がなくなるから注意が必要。
...
bufio.Scannerでは、デフォルトは改行区切りだが、分割関数を指定することで任意の分割が行える。

### 3.6.2 データ型を指定して解析

io.Reader のデータを整数や浮動小数点数に変換するには、fmt.Fscan() を使います

1つめの引数にio.Reader を渡し、それ以降に変数のポインタを渡すと、その変数にデータが書き込まれる

- fmt.Fscan() はデータがスペース区切りであることを前提
- fmt.Fscanln()は改行区切り時に使う
- fmt.Fscanf() を使うと任意のデータ区切りをフォーマット文字列として指定できる
...
- csv.Readerはio.Readerを受け取り
- Read()メソッドを呼ぶと、行の情報（文字列の配列）を返す
  - ReadAll() で、それがさらに配列になったものを一度に返すこともできる

encoding/json はさまざまな使い方ができる複雑なライブラリなので、扱わない。
